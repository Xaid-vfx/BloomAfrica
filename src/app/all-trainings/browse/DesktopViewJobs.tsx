...
async function getJobs(page: number = 1, pageSize: number = 15) {
    ...
    // First, get Individual/Artisan trainer user IDs
    const { data: individualTrainers } = await supabase
        .from('TrainerProfiles')
        .select('user_id')
        .eq('trainer_category', 'Individual/Artisan/Small Business')
        .eq('is_completed', true)

    const trainerUserIds = individualTrainers?.map(t => t.user_id) || []

    // If no Individual/Artisan trainers found, return empty
    if (trainerUserIds.length === 0) {
        return { data: [], count: 0 };
    }

    // Get jobs from Individual/Artisan trainers only
...
export default function DesktopViewJobs(props: any) {
    ...
    const handleJobSelect = (jobId: string) => {
        ...
        router.push(`/all-trainings/browse?selected=${jobId}`, { scroll: false } as any)
    }

    const handleHowToSelect = () => {
        setSelectedJobId('howto')
        router.push(`/all-trainings/browse?selected=howto`, { scroll: false } as any)
    }
    ...
    useEffect(() => {
        if (!selectedJobId) {
            if (currentPage === 1) {
                ...
                router.push(`/all-trainings/browse?selected=${firstJobId}`, { scroll: false } as any)
            }
        }
